...
- **2026-09-16**: **Landing page rewrite** — problem-first / help-vs-tutor narrative (`app/page.jsx` + new `src/components/landing/Landing{Problem,HoursLost,FreeVsPaid,VsTutor,BookCall,WrittenBy,StartFree}.jsx`). Interactive "hours you've lost" calculator (live-total inputs), a real comparison table, and a **Calendly** "book a free call" embed (`calendly.com/jia432h/30min`, loaded via next/script; Calendly added to the staged CSP). Kept the course list, founder quote, price, FAQ, and all JSON-LD; dropped the old "Why this exists" / "What this is" sections (now unused: `WhyThisExists.jsx`, `WhatThisIs.jsx`). Hero H1 → "IB Extended Essay help".
