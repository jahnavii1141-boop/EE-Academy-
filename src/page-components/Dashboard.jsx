    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {isLocked && <Lock className="w-4 h-4 text-navy/40" strokeWidth={1.8} />}
          {!isLocked && progressPercent === 100 && <CheckCircle className="w-4 h-4 text-green-500" strokeWidth={1.8} />}
        </div>
      </div>

      <div className="mb-4">
        <h3 className="font-serif text-base font-bold text-navy leading-snug mb-1">{section.title}</h3>
      ...
      <div className="mb-4 space-y-1.5">
        ...
        {section.stepLabels.length > 3 && (
          <div className="text-xs text-navy/40 pl-4">+{section.stepLabels.length - 3} more</div>
        )}
      </div>

      <div className="mb-4">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[10px] font-medium text-navy/40 uppercase tracking-wide">Progress</span>
          <span className="text-[10px] font-semibold text-navy/50">{progressPercent}%</span>
        </div>
        <div className="h-1.5 bg-navy/10 rounded-full overflow-hidden">
          <div className="h-full bg-navy/40 rounded-full transition-all duration-500" style={{ width: `${progressPercent}%` }} />
        </div>
      </div>

      <div className="flex items-center justify-between">
        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${badge.className}`}>{badge.label}</span>
    ...
    <div className="min-h-screen bg-cream">
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-8">
        ...
        <p className="text-sm text-ink-soft max-w-lg">
          Free Resource Lab · 14 modules · Built by a real 32/34 IB student. Work through each section in order for the best results.
        </p>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-24">
        ...
        <div className="mt-10 rounded-2xl bg-navy px-6 py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <p className="font-serif text-base font-bold text-cream mb-1">Ready to unlock everything?</p>
            <p className="text-sm text-steel">Get lifetime access to all 14 modules, templates, tools &amp; AI prompts.</p>
          </div>
          <Link href="/pricing" className="flex-shrink-0 bg-cream text-navy font-semibold text-sm px-5 py-2.5 rounded-xl hover:bg-parchment transition-colors">
            Buy Full Access →
          </Link>
        </div>
      </div>
