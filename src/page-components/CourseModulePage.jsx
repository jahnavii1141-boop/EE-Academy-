    ...
    <nav className="flex flex-col h-full">
      ...
      <div className="border-t border-navy/6 px-6 py-4">
        ...
        <div className="h-1 bg-navy/6 rounded-full">
          <div className="h-full bg-navy/25 rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
        </div>
      </div>
    </nav>
  )
    ...
    return (
      <div className="min-h-screen bg-cream flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/courses" className="btn-primary">Back to Resource Lab</Link>
        </div>
      </div>
    )
  }
      ...
      <div className="sticky top-14 z-30 bg-cream/90 backdrop-blur-sm border-b border-navy/6">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-2.5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => setDrawerOpen(true)} className="lg:hidden text-navy/40 hover:text-navy p-1 -ml-1" aria-label="Open module list">
              <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 5a1 1 0 011-1h6a1 1 0 110 2H4a1 1 0 01-1-1z" clipRule="evenodd" />
              </svg>
            </button>
            <Link href="/courses" className="flex items-center gap-1.5 text-sm text-navy/40 hover:text-navy transition-colors">
          ...
          <div className="flex lg:hidden items-center gap-1">
            {COURSE_MODULES.map((m, i) => (
              <Link key={m.id} href={`/course/${m.id}`}>
                <span title={m.title} className={`block w-1.5 h-1.5 rounded-full transition-all ${i === moduleIndex ? 'bg-navy scale-125' : i < moduleIndex ? 'bg-navy/30' : 'bg-navy/10'}`} />
              </Link>
            ))}
          </div>
          <span className="text-xs text-navy/35 tabular-nums">{moduleIndex + 1} / {COURSE_MODULES.length}</span>
        </div>
        <div className="h-px bg-navy/5 lg:hidden">
          <div className="h-full bg-navy/20 transition-all duration-500" style={{ width: `${((moduleIndex + 1) / COURSE_MODULES.length) * 100}%` }} />
        </div>
      </div>
          ...
          <div className="max-w-3xl mx-auto px-6 pt-12 pb-20">
            ...
            {(!isGated || module.free) && (
              <div className="border-t border-navy/6 mt-16 pt-8 flex items-stretch justify-between gap-4">
                {prevModule ? (
                  <Link href={`/course/${prevModule.id}`} className="flex items-center gap-3 group rounded-lg border border-navy/8 hover:border-navy/15 hover:bg-parchment/20 transition-all px-5 py-4 flex-1 max-w-[48%]">
                ...
                {nextModule ? (
                  <Link href={`/course/${nextModule.id}`} className="flex items-center justify-end gap-3 group rounded-lg border border-navy/8 hover:border-navy/15 hover:bg-parchment/20 transition-all px-5 py-4 flex-1 max-w-[48%] text-right">
                    <span>
